...
import togawasakikomod.Actions.MaskAction;
...
public class Masks extends BaseCard {
    public static final String ID = makeID(Masks.class.getSimpleName());
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        addToBot(new PlayAudioAction(Masks.class.getSimpleName()));

        addToBot(new ReducePowerAction(p,p, DazzlingPower.POWER_ID,magicNumber));
        addToBot(new ApplyPowerAction(p,p,new StrengthPower(p,this.magicNumber),this.magicNumber));
        addToBot(new MaskAction(1,this));
